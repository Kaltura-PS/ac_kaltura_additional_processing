...
while IFS=, read -r ENTRY_ID SCO_ID MEETING_ID <&3 ;do
        set -o nounset
        ...
        nohup sh -c "xvfb-run-safe -s \"-auth /tmp/xvfb.auth -ac -screen 0 1280x720x24\" $BASEDIR/additional_processing_per_entry.rb $ENTRY_ID $SCO_ID " > /tmp/ac_$ENTRY_ID 2>&1 &
        sleep 2
